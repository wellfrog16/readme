...
({
    ...
    // 包含应用程序的顶层目录。如果使用此选项，则假定您的脚本位于此路径下的子目录中。
    // 这个选项不是必需的。如果没有指定，那么下面的baseUrl就是找到东西的锚点。
    // 如果指定了此选项，则应用程序目录中的所有文件将被复制到dir: output area,
    // baseUrl将假定为该目录下的相对路径。
    ...
    // 默认情况下，所有模块都位于相对于此路径的位置。
    // 如果baseUrl没有显式设置，那么所有模块都相对于保存构建文件的目录加载。
    // 如果设置了appDir，那么应该将baseUrl指定为相对于appDir的。
    ...
    // 默认情况下，所有优化配置都发生在命令行或配置文件中的属性中，
    // 而作为应用程序运行时“main”JS文件的一部分，被传递给requirejs的配置则不被考虑。
    // 但是，如果您更喜欢为构建而读取的“main”JS文件配置，这样您就不必在单独的配置中复制这些值，
    // 将该属性设置为该主JS文件的位置。
    // 第一个requirejs({})， require({})， requirejs.config({})，或require.config({})调用将被使用。
    // 在2.1.10中，mainConfigFile可以是一个值数组，最后一个值的配置优先于数组中先前的值。
    ...
    // 设置路径模块。如果相对路径，设置相对于上面的baseUrl。
    // 如果"empty:"的特殊值被用于路径值，那么就像映射到空文件的路径。
    // 它允许优化器解决对路径的依赖关系，但不包括在输出中。
    // 当在浏览器中运行时，在浏览器中运行时，可以将这些模块名映射到资源上，并在优化中忽略该文件，因为它没有依赖关系。
    // 如果您希望将“jquery”和“angularjs”从公共的CDNs中包含进来，
    // paths: { "jquery": "empty:", "angular": "empty:" }
    ...
    // 设置模块id到其他模块id的映射。
    // 更多细节,请参见http://requirejs.org/docs/api.html#config-map。
    ...
    // 配置CommonJS包。
    // 有关更多信息,请参见 http://requirejs.org/docs/api.html#packages。
    ...
    // 要保存输出的目录路径。
    // 如果没有指定，那么路径将默认为一个名为“build”的目录，作为构建文件的兄弟。
    // 所有相对路径都相对于构建文件。
    ...
    // 在RequireJS 2.0.2中，上面的dir将在构建重新开始之前被删除。
    // 如果你有一个大的构建和源转换onBuildRead / onBuildWrite没有做,
    // 那么你可以设置keepBuildDir真正保持前面的dir。
    // 这允许更快的重新构建，但是如果构建的代码在某种程度上发生了转换，它可能会导致意外的错误。
    ...
    // 如果在运行时在应用程序中使用了shim配置，在这里复制配置。
    // 如果使用shim配置，则需要将shim的依赖项包含在构建中。
    // 使用“mainConfigFile”是传递此信息的更好方法，因此它只在一个地方列出。
    // 但是，如果mainConfigFile不是一个选项，那么shim配置可以在构建配置中内联。
    ...
    // 在2.1.11中，shimmed依赖项可以封装在一个define()包装器中，
    // 以帮助中间依赖关系是AMD自己的依赖关系。
    // 典型的例子是一个使用主干的项目，它依赖于jQuery和Underscore。
    // 在构建中，希望主干立即可用的Shimmed依赖项不会在构建中看到它，
    // 因为AMD兼容的主干版本将不会执行define()函数，直到依赖关系就绪为止。
    // 通过包装那些闪烁的依赖项，可以避免这种情况，
    // 但是如果这些shimmed依赖项以怪异的方式使用全局范围，那么它可能会引入其他错误，所以这不是默认的行为。
    // 使用shim包装的skipmodule插入需要是错误的。
    // 信息 http://requirejs.org/docs/api.html#config-shim
    ...
    // 用于将i18n资源内联到构建文件中。
    // 如果没有指定语言环境，那么i18n资源将不会被内联。
    // 只有一个地区可以内联进行构建。构建层引用的根包将被包含在构建层中，而不考虑设置区域。
    locale: "en-us",
    ...
    // 如何在构建输出目录中optimize (minify)所有JS文件。
    // 现在只有以下值。    
    // 支持:    
    // - "uglify": (default)使用 UglifyJS 来缩小代码。
    // 在版本2.2之前，uglify版本是1.3。x版本。与r。js 2.2，现在是2。x糟蹋。
    // 只支持ES5的语法。在2015年或以后，使用“none”选项代替。
    
    // - "uglify2":版本2.1.2+。
    // 使用UglifyJS2。的r。js 2.2，这只是“uglify”的别名，现在2.2只使用了uglify 2.x。
    
    // - "closure":在简单的优化模式下使用谷歌的闭包编译器来缩小代码。只有在使用Java运行优化器时才可用。
    
    // - "closure.keepLines":与关闭选项相同，但在缩小的文件中保留行返回。
    
    // - "none":不会缩小。如果您在文件中使用的是ES 2015或以后的语法，那么使用这个设置，因为UglifyJS绑定只理解ES5和更早的语法。
    // 对于ES2015代码，在运行r.js之后，运行一个兼容的minifier作为独立的步骤。
    ...
    // 在2.1.2中引入:如果使用“dir”作为输出目录，通常使用优化设置来优化构建包(配置的"modules"部分)和目录中的任何其他JS文件。
    // 但是，如果在构建之后不加载非构建包JS文件，则可以跳过这些文件的优化，以加速构建。
    // 如果您想跳过对其他非构建包JS文件的优化，请将该值设置为true。
